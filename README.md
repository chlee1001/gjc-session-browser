...
기본으로 **최근 1주**만 봅니다. 사이드바 기간 드롭다운으로 오늘·3일·1주·1개월·3개월·전체를 고르거나, 직접 지정으로 시작·종료 날짜를 잡습니다. 위 통계와 작업 폴더 목록도 고른 기간을 따릅니다. 상태로 걸러볼 때만 예외로 기간을 무시합니다 — 표시해 둔 세션이 기간 밖으로 밀려 사라지면 기능이 무의미해집니다.

세션에는 **작업 중** 과 **완료** 표시를 달 수 있습니다. 둘은 배타적이라 하나를 켜면 다른 하나는 풀립니다. 끝났지만 지우기는 아까운 세션을 완료로 빼놓고, 사이드바에서 상태별로 추려 봅니다. 표시 자체는 세션 원본이 아니라 별도 설정 파일에 저장합니다.
...
| 메서드 | 경로 | 설명 |
| --- | --- | --- |
| `GET` | `/api/sessions` | 목록. `q`, `folder`, `status=active\|done`, `from`·`to`(UTC ISO), `offset`, `limit`(≤100), `summaryOnly=1`, `refresh=1` |
