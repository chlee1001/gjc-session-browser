...
통계 아래 **모델별 사용량** 은 같은 기간의 토큰을 모델로 쪼갠 표입니다. 토큰·비중·비용·응답 수·세션 수를 모델마다 보여 줍니다.
...
`/api/sessions` 응답의 `summary.models` 는 기간 안의 토큰·비용을 모델별로 나눈 배열입니다. 세션마다 붙는 `models` 는 그 세션 몫입니다. 둘 다 토큰 내림차순이고 서브에이전트를 포함합니다. 서브에이전트 몫만 따로 보려면 세션의 `subagentTokens` · `subagentCost` 를 쓰세요.
...
`session-scanner.test.js` 는 JSONL 파싱·필터·`header_patch` 반영, 모델을 갈아탄 세션의 모델별 분해, 서브에이전트 전사본 합산, 전사본이 늘었을 때의 캐시 무효화, 중복 사본 제거를 검증합니다. `server.test.js` 는 임시 홈 디렉터리와 임시 세션 파일로 서버를 띄워 제목 변경 → 잘못된 확인값 거부 → 삭제 → 설정 정리까지 실제 API로 확인합니다. 사용자의 실제 세션은 건드리지 않습니다.
