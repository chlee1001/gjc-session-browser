...
172개 세션 / Apple M3 Pro / 캐시가 있는 상태:
...
캐시가 없는 첫 실행은 전체 파싱이 필요해 더 걸립니다.

세션 파일 `<이름>.jsonl` 옆의 같은 이름 디렉터리는 그 세션의 아티팩트 폴더입니다. 안에 서브에이전트 트랜스크립트가 `.jsonl` 로 들어 있지만 독립 세션이 아니므로 탐색에서 제외합니다. 이 저장소에서는 전체 `.jsonl` 3,019개 중 실제 세션이 177개입니다.
...
| `~/.gjc/session-list.json` | 추가한 세션 경로, 세션별 상태(작업 중/완료) | 지우면 표시와 경로 설정이 사라집니다 |
...
| 메서드 | 경로 | 설명 |
| --- | --- | --- |
| `GET` | `/api/sessions` | 목록. `q`, `folder`, `status`(쉼표 구분, `none`·`active`·`done`), `from`·`to`(UTC ISO), `offset`, `limit`(≤100), `summaryOnly=1`, `refresh=1` |
