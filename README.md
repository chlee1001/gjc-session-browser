...
기본으로 **최근 1주**만 봅니다. 사이드바 기간 드롭다운으로 오늘·3일·1주·1개월·3개월·전체를 고르거나, 직접 지정으로 시작·종료 날짜를 잡습니다. 위 통계와 작업 폴더 목록도 고른 기간을 따릅니다. 작업 중 표시만 예외로 기간을 무시합니다 — 북마크가 기간 밖으로 밀려 사라지면 기능이 무의미해집니다.
...
| 메서드 | 경로 | 설명 |
| --- | --- | --- |
| `GET` | `/api/sessions` | 목록. `q`, `folder`, `focus=1`, `from`·`to`(UTC ISO), `offset`, `limit`(≤100), `summaryOnly=1`, `refresh=1` |
